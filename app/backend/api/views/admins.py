...
class AdminViewSet(viewsets.GenericViewSet):
    ...
    def delete_comment(self, request):
        ...
        # we try to get the comment here with id.
        is_found = Comment.objects.filter(id=comment_id)
    ...
    def delete_user_by_comment_id(self, request):
        ...
        try:
            comment = Comment.objects.get(
                id=comment_id)  # try to get the comment
        except:
            return Response(data={'error': 'Comment is not found with that id'}, status=status.HTTP_400_BAD_REQUEST)
        # getting the owner of the comment
        user_with_that_commnet_id = comment.customer.user
        social_docs = SocialDocs.objects.filter(
            email=user_with_that_commnet_id.email)
        if social_docs:
            social_docs.delete()
        # deleting
    ...
    def delete_user_by_username(self, request):
        ...
        social_docs = SocialDocs.objects.filter(email=user_get.email)
        if social_docs:
            social_docs.delete()
        # deleting the user
        user.delete()
        return Response(data={"success": "user is deleted"}, status=status.HTTP_200_OK)
